...
TransformMatrix
TransformMatrix::rotation(double angle, const Point & center, Type type )
{
  return TransformMatrix::translation(-center)
      * TransformMatrix::rotation(angle,type)
      * TransformMatrix::translation(center);
}
...
Point
TransformMatrix::operator*(const Point & point ) const
{
  return Point( _m11*point.x + _m12*point.y + _m13,
                _m21*point.x + _m22*point.y + _m23 );
}
...
TransformMatrix &
TransformMatrix::operator+=(const Point & point)
{
  _m13 += point.x;
  _m23 += point.y;
  return *this;
